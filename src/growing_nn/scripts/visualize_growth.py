"""
Visualize how a series of saved growth checkpoints (``0.pt``, ``1.pt``,
...) grew, block by block and sub-module by sub-module (Q/K/V/proj/fc1/fc2).

Equivalent to running the original ``python vis_growth.py <folder>``.
Once this package is installed, run:

    python -m growing_nn.scripts.visualize_growth <folder>

NOTE ON BEHAVIOR CHANGE
------------------------
The original script imported ``from growth_utils_node import
get_all_linear_layers`` — a module that does not exist anywhere in the
project (the actual growth-utilities module is
``growth_utils_node_new.py`` / :mod:`growing_nn.growth`). This would have
raised ``ModuleNotFoundError`` immediately. That import is fixed below to
point at :func:`growing_nn.growth.get_all_linear_layers`.
"""
import argparse
import os
import shutil
...
from growing_nn.growth import get_all_linear_layers
...
def plot_param(param, path, p):
    ...
    sns.barplot(
        x="block",
        y="parameters",
        data={"block": [i for i in range(len(param))], "parameters": param},
        palette=sns.color_palette("rocket"),
    )
    n = path.split("/")[-1].split(".")[0]
