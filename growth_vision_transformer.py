...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Linear(in_features, int(hidden_features/2))
        self.act = act_layer()
        self.fc2 = nn.Linear(int(hidden_features/2), out_features)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.q = nn.Linear(dim, int(dim/2), bias=qkv_bias)
        self.k = nn.Linear(dim, int(dim/2), bias=qkv_bias)
        self.v = nn.Linear(dim, int(dim/2), bias=qkv_bias)
    ...
    def forward(self, x):
        B, N, C = x.shape
        C=int(C/2)
        qk = torch.cat([self.q(x),self.k(x)],axis=2)
        C= int(torch.flatten(qk).shape[0]/(B*N*2))
        qk = qk.reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        #qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k= qk[0], qk[1]   # makesl torchscript happy (cannot use tensor as tuple)
        v_out= self.v(x)
        VC =int(torch.flatten(v_out).shape[0]/(B*N))
        v = v_out.reshape(B, N, self.num_heads, VC // self.num_heads).permute( 0, 2, 1, 3)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, VC)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
@register_model
def vit_tiny_patch16_224_16_heads(pretrained=False, **kwargs):
    if pretrained:
        # NOTE my scale was wrong for original weights, leaving this here until I have better ones for this model
        kwargs.setdefault('qk_scale', 768 ** -0.5)
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3., **kwargs)
    model.default_cfg = default_cfgs['vit_tiny_patch16_224_16_heads']
    if pretrained:
        load_pretrained(
